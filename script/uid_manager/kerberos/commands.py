...
def build_host_refresh_command(config: AppConfig, username: str, uid: int, gid: int, paths: KerberosPaths) -> str:
    ...
    return "\n".join([
        ...
        "  fi",
        "fi",
        "if [ \"$should_reissue\" = \"false\" ] && klist -s -c \"$DECS_KRB_CCACHE\" 2>/dev/null && timeout \"$kinit_timeout\" kinit -R -c \"$DECS_KRB_CCACHE\" >/dev/null 2>&1; then",
        "  :",
        "else",
        ...
        f"if ! {sudo}timeout 60 systemctl start decs-krb-refresh@{username}.service; then",
        f"  if {sudo}klist -s -c {q('FILE:' + paths.ccache_file)} 2>/dev/null; then",
        "    echo kerberos_host_refresh_existing_ccache_ok",
        "  else",
        f"    {sudo}systemctl status decs-krb-refresh@{username}.service --no-pager -l >&2 || true",
        "    exit 1",
        "  fi",
        "fi",
    ])
