...
`uidctl` CLI. It mirrors the previous shell-script Kerberos test and adds
...
cd ~/uid
bash legacy/tests/test_farm_nas_home.sh
bash legacy/tests/test_manage_group.sh
```
...
- Expired cleanup/email:
  - Added safe filters `--username` and `--container-name` to the Python CLI so E2E cleanup does not delete unrelated expired FARM containers.
