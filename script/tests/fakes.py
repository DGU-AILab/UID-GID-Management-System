...
class FakeRepository:
    def __init__(self) -> None:
        ...
        self.kerberos_identities: dict[str, KerberosIdentityRecord] = {}
    ...
    def upsert_kerberos_identity(self, record: KerberosIdentityRecord) -> None:
        self.kerberos_identities[record.username] = record
...
class FakeAnsibleRunner:
    ...
    def shell(self, host: str, command: str, check: bool = True) -> CommandResult:
        ...
        if "stat -c '%u %g'" in command:
            return CommandResult(["ansible", host], stdout=f"{self.last_ad_uid} {self.last_ad_gid}\n")
        stdout = self.shell_outputs.pop(0) if self.shell_outputs else ""
