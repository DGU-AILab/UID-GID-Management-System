...
function show_help {
  ...
  echo "      --no-db-record              Create the Docker container but skip DB user/group/container/port writes"
  echo ""
...
echo "  DB Record: $([ "$no_db_record" = "true" ] && echo "disabled" || echo "enabled")"
...
function cleanup_and_exit {
  ...
  if [ "${db_transaction_started:-false}" = "true" ] && [ -n "${MYSQL_CNF_FILE:-}" ] && [ -f "$MYSQL_CNF_FILE" ]; then
...
if [ "$dry_run" = "true" ]; then
  ...
  if [ "$no_db_record" = "true" ]; then
    echo "[DRY-RUN] Would skip DB user/group/container/port records (--no-db-record)"
    echo "[DRY-RUN] Would send creation email, but skip DB backup and export refresh"
  else
...
if [ "$no_db_record" = "true" ]; then
  echo "Created container ${container_name_param} on ${target_host} without writing DB records (--no-db-record)."
  ...
  if ! python3 "${PROJECT_ROOT}/script/send_container_created_email.py" \
    ...
    log_error "creation_notification_failed username=${username} server=${server_id} no_db_record=true"
  fi

  echo "Skipping DB backup and export refresh because no DB record was written."
  exit 0
fi
