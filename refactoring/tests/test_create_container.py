...
from uid_manager.models import CreateContainerRequest, GroupRecord, UserRecord
...
def test_dry_run_kerberos_container_sets_ccache_and_restricted_sudo():
    ...
    assert "--mount type=bind,source='/mnt/nas-krb-test-v4/user-share/',target=/home" in command
...
def test_apply_kerberos_runs_ad_nas_refresh_ccache_and_access_check():
    ...
    assert result.runtime_gid == 96470003
    ...
    assert "setpriv --reuid=10000 --regid=96470003" in shell_commands
    docker_runs = [command for _, command in remote.shell_calls if command.startswith("docker run")]
    assert docker_runs and "-e GID='96470003'" in docker_runs[0]
